...
type UidDumper interface {
	DumpUid(tcp bool, ipv6 bool, srcIp string, srcPort int, destIp string, destPort int) (int, error)
}
...
func (t *Tun2socks) DialContext(ctx context.Context, metadata *constant.Metadata) (net.Conn, error) {
	uid, err := uidDumper.DumpUid(true, len(metadata.DstIP) == net.IPv6len, metadata.SrcIP.String(), int(metadata.SrcPort), metadata.DstIP.String(), int(metadata.DstPort))
...
func (t *Tun2socks) DialUDP(metadata *constant.Metadata) (net.PacketConn, error) {
	uid, err := uidDumper.DumpUid(false, len(metadata.DstIP) == net.IPv6len, metadata.SrcIP.String(), int(metadata.SrcPort), metadata.DstIP.String(), int(metadata.DstPort))
	isDns := metadata.DstIP.String() == t.router || metadata.DstPort == 53 && t.hijackDns
	var destProxy *proxy.Socks5
	if err != nil {
		log.Warnf("dumpUid failed: %v", err)
		...
		if rule == 1 {
			...
			return udpFileConn{Conn: conn, addr: metadata.UDPAddr()}, nil
		} else if rule > 1 && !isDns {
			destProxy = t.uidProxies[uid]
		}
	}
